...
void load_bpf_and_xdp_attach(int ifindex, const char* filename, const char* section)
{
...
int main(int argc, char* argv[])
{
  ...
  int i;
  for (i = 0; i < sizeof(args) / sizeof(args[0]) && i + 1 < argc; i++) {
    *args[i] = argv[i + 1];
  }
  ...
  load_bpf_and_xdp_attach(ifindex, file, sec);
